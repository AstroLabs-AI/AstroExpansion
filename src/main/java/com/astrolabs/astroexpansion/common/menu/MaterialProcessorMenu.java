...
public class MaterialProcessorMenu extends AbstractContainerMenu {
    ...
    public MaterialProcessorMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }
    
    public MaterialProcessorMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.MATERIAL_PROCESSOR_MENU.get(), id);
        checkContainerSize(inv, 2);
        this.blockEntity = (MaterialProcessorBlockEntity) entity;
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
            player, ModBlocks.MATERIAL_PROCESSOR.get());
    }

...
import com.astrolabs.astroexpansion.common.blockentities.TieredMaterialProcessorBlockEntity;
...
public class TieredMaterialProcessorMenu extends AbstractContainerMenu {
    private final TieredMaterialProcessorBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;
    private final MachineTier tier;
    
    public TieredMaterialProcessorMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4), MachineTier.BASIC);
    }
    
    public TieredMaterialProcessorMenu(int id, Inventory inv, BlockEntity entity, ContainerData data, MachineTier tier) {
        super(ModMenuTypes.TIERED_MATERIAL_PROCESSOR.get(), id);
        checkContainerSize(inv, 4);
        this.blockEntity = (TieredMaterialProcessorBlockEntity) entity;
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        int inventoryStart = 2 + tier.getUpgradeSlots();
        ...
        // From player inventory
            ...
            if (!this.moveItemStackTo(stack, 0, 1, false)) {
                ...
                if (blockEntity.isValidUpgrade(stack)) {
                    if (!this.moveItemStackTo(stack, 2, 2 + tier.getUpgradeSlots(), false)) {
                        return ItemStack.EMPTY;
                    }
                } else {
                    return ItemStack.EMPTY;

...
import com.astrolabs.astroexpansion.common.recipe.ProcessingRecipe;
import com.astrolabs.astroexpansion.common.recipe.ProcessingRecipeRegistry;
...
public class MaterialProcessorBlockEntity extends AbstractMachineBlockEntity implements Container, MenuProvider {
    ...
    public MaterialProcessorBlockEntity(BlockPos pos, BlockState state) {
        super(AEBlockEntities.MATERIAL_PROCESSOR.get(), pos, state, ENERGY_CAPACITY);
    }
    ...
    public void tick() {
        ...
        if (currentRecipe == null || !currentRecipe.matches(items.get(0))) {
            currentRecipe = ProcessingRecipeRegistry.findRecipe(items.get(0)).orElse(null);
            processTime = 0;
        }
        
        if (currentRecipe != null && canProcess() && energyStorage.getEnergyStored() >= currentRecipe.getEnergyCost()) {
            isActive = true;
            ...
            energyStorage.extractEnergy(currentRecipe.getEnergyCost() / currentRecipe.getProcessTime(), false);
            processTime++;
    ...
    private boolean canProcess() {
        if (currentRecipe == null || items.get(0).isEmpty()) return false;
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, MaterialProcessorBlockEntity blockEntity) {
        blockEntity.tick();
    }
    ...
    public AbstractContainerMenu createMenu(int containerId, Inventory playerInventory, Player player) {
        return new MaterialProcessorMenu(containerId, playerInventory, this, getContainerData());
    }
